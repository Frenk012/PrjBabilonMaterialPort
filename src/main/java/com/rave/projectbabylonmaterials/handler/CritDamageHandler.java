...
public final class CritDamageHandler {
    ...
    private static final float CRIT_RESISTANCE_MULTIPLIER = 0.5F;
    ...
    public static void onLivingDamage(LivingDamageEvent event) {
        ...
        float finalDamage = (float) (event.getAmount() * (1.0D + critDamage));
        if (event.getEntity().hasEffect(PBMEffects.CRIT_RESISTANCE.get())) {
            finalDamage *= CRIT_RESISTANCE_MULTIPLIER;
        }
