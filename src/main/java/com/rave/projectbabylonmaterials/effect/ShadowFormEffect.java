...
public class ShadowFormEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (isConcealed(entity)) {
            ...
            if (nextAfterimageTick <= 0L) {
                ...
                PBNetwork.CHANNEL.send(PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity), new ClientboundShadowFormAfterimagePacket(entity.getId()));
                entity.getPersistentData().putLong(NEXT_AFTERIMAGE_TICK_KEY, gameTime + AFTERIMAGE_INTERVAL_TICKS);
            }
            return;
        }
    ...
    public static void onEffectAdded(MobEffectEvent.Added event) {
        if (event.getEffectInstance().getEffect() != PBMEffects.SHADOW_FORM.get() || event.getEntity().level().isClientSide()) {
            return;
        }
    ...
    public static void onEffectRemoved(MobEffectEvent.Remove event) {
        if (event.getEffect() != PBMEffects.SHADOW_FORM.get() || event.getEntity().level().isClientSide()) {
            return;
        }
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (target.hasEffect(PBMEffects.SHADOW_FORM.get())) {
            breakConcealment(target);
        }

        if (event.getSource().getEntity() instanceof LivingEntity attacker && attacker.hasEffect(PBMEffects.SHADOW_FORM.get())) {
    ...
    public static void onLivingTick(LivingEvent.LivingTickEvent event) {
        ...
        if (!entity.hasEffect(PBMEffects.SHADOW_FORM.get()) && hasShadowFormState(entity)) {
    ...
    private static void breakConcealment(LivingEntity entity) {
        if (entity.level().isClientSide() || !entity.hasEffect(PBMEffects.SHADOW_FORM.get())) {
            return;
        }
    ...
    private static void refreshInShadows(LivingEntity entity) {
        entity.addEffect(new MobEffectInstance(PBMEffects.IN_SHADOWS.get(), IN_SHADOWS_DURATION_TICKS, 0, false, false, true));
    }
    ...
    private static void clearShadowFormState(LivingEntity entity) {
        ...
        entity.removeEffect(PBMEffects.IN_SHADOWS.get());
        syncConcealment(entity, false);
    }

    private static void syncConcealment(LivingEntity entity, boolean concealed) {
        PBNetwork.CHANNEL.send(PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity), new ClientboundShadowFormStatePacket(entity.getId(), concealed));
    }
}